...
def transform_decode(xs: np.ndarray):
    ub = max(1, len(xs) - block_size * 3 // 5) # ub >= 1
    start = torch.randint(ub, (1,)).item()
    n = min(block_size * 3 // 5, len(xs))
    encoded = lz.encode(xs[start:start+n])
    eot = np.asarray([end_token_id], dtype=encoded.dtype)
    decoded = xs[start:start+n].astype(encoded.dtype)
    x = torch.from_numpy(np.concatenate([encoded, eot, decoded]))
    n = min(block_size + 1, len(x))
    return x[:n-1], x[1:n]


def transform_next_token(xs: np.ndarray):
...
def main():
    ...
    owt_dataset = load_dataset("openwebtext", num_proc=None)
    split_dataset = owt_dataset["train"].train_test_split(test_size=0.0005, seed=2357, shuffle=True)
    ...
    train_gen = get_batch_continuous(trainloader)
    val_gen = get_batch_continuous(valloader)
